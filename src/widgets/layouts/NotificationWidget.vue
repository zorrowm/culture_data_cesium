<template>
  <q-btn flat round size="sm" icon="mdi-bell-outline">
    <q-badge color="negative" style="padding: 2px 4px" title-color="white" floating>{{
      totalInformCount
    }}</q-badge>
        ...
        <q-card class="full-width">
          ...
          <q-tab-panels v-model="messageTab" animated>
            <q-tab-panel name="inform" class="q-pa-none">
              <q-list>
                <div
                  v-for="(inform, index) in informList"
                  v-bind:key="index"
                ...
                >
                  ...
                  <q-separator v-if="index < informList.length - 1" inset="item" />
                </div>
                <q-separator />
            ...
            <q-tab-panel name="waitDeals" class="q-pa-none">
              <q-list>
                <div
                  v-for="(waitDeal, index) in waitDealList"
                  v-bind:key="index"
...
function look(type, index) {
  if (type === 'inform') {
    informList[index].disable = true;
  } else if (type === 'notify') {
    notifyList[index].disable = true;
  } else {
    waitDealList[index].disable = true;
  }
}
...
//   };
function learMessage(items) {
  if (items === 'inform') {
    disable(informList);
  } else if (items === 'notify') {
    disable(notifyList);
  } else {
    disable(waitDealList);
  }
}
...
const informCount = computed(() => {
  return getAvailableCount(informList);
});
const notifyCount = computed(() => {
  return getAvailableCount(notifyList);
});
const waitDealCount = computed(() => {
  return getAvailableCount(waitDealList);
});
const totalInformCount = computed(() => {
  return informCount.value + notifyCount.value + waitDealCount.value;
});
</script>
