  ...
  <div class="row no-wrap full-width justify-center items-center">
    ...
    <div class="q-gutter-xs row no-wrap items-center">
      <slot name="right">
        ...
        <q-input class="inputSize" outlined v-model="typeValue" dense placeholder="类型搜索" debounce="300" @update:model-value="doClick('searchWord')" />
        
        <q-input class="inputSize" v-if="isArea" outlined v-model="areaValue" dense placeholder="区域搜索" debounce="300" @update:model-value="doClick('searchWord')"  />
...
const doClick = (val: string) => {
  if (val === 'searchWord') {
    if (dateValue.value && endDateValue.value) {
      ...
      emit('topBarClick', val, searchValue.value, typeValue.value,areaValue.value, dateValue.value, endDateValue.value);
    } else {
      emit('topBarClick', val, searchValue.value, typeValue.value,areaValue.value);
    }
  } else {
